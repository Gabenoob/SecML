...
if __name__ == "__main__":
    ...
    for i in range(len(classes_list)):
        le.append(LabelEncoder().fit(data[:, i]))
    ...
    # 使用模型预测
    dataTest = pd.DataFrame(['青绿','蜷缩','沉闷','清晰','凹陷','硬滑','否'], index=classes_list).T
    dataTest = dataTest.values
    for i in range(len(le)):
        dataTest[:,i] = le[i].transform(dataTest[:,i])
    test_example = dataTest[:,:-1]      # 测试样例
